...
Subroutine CheckHbonds(nmol, ntimes, Hbonds, intact)
    ...
    integer, dimension(2000,500) :: intact
    integer, dimension(2000,1000,6) :: Hbonds
    integer, dimension(1000,6) :: track
        ...
        do t=1,ntimes
            do k=2,6
                if ( Hbonds(t,j,k) .ne. Hbonds(1,j,k) ) then
                    if ( Hbonds(1,j,k) .eq. 0 .and. track(j,k) .eq. 0 ) then
                        track(j,k) = Hbonds(t,j,k)
                        write(*,*) "track",j,k,t
                    endif
                    if ( Hbonds(t,j,k) .ne. 0 ) then ! Make sure that not transient at t
                        ! if transient at t=0, checks against value 
                        if ( track(j,k) .ne. 0 .and. Hbonds(t,j,k) .ne. track(j,k)) then
                            flag=0
                        elseif ( track(j,k) .eq. 0 ) then 
                            flag=0
                        endif
                    endif
                endif
